...
'''
    If you need values from Python variables it is recommended to use the "?" placeholder. Never use string operations or
    concatenation to make your queries because is very insecure.
    '''
cursor = db.cursor()
name = 'Andres'
phone = '3366858'
...
````python
    '''
    The values of the Python variables are passed inside a tuple. Another way to do this is passing a dictionary using
    the ":keyname" placeholder:
    '''
cursor = db.cursor()
cursor.execute('''INSERT INTO users(name, phone, email, password)
    VALUES(:name,:phone, :email, :password)''', {'name': name, 'phone': phone, 'email': email, 'password': password})
db.commit()
````
...
# use fetchall() to retrieve all the rows.
...
UPDATE SQLITE_SEQUENCE SET seq = 0 WHERE name = 'customer';
````
...
user_id = 3 cursor.execute('''SELECT name, email, phone FROM users WHERE id=?''', (user_id,))
...
Transactions are an useful property of database systems. It ensures the atomicity of the Database. Use commit to save
the changes. Or rollback to roll back any change to the database since the last call to commit:
'''
...
db = sqlite3.connect('db.sqlite3')
cursor = db.cursor()
cursor.execute('''CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY, name TEXT, phone TEXT, email TEXT, password
TEXT)''')
db.commit()
except Exception as e:

# This is called a catch-all clause. # This is used here only as an example. # In a real application you should catch a

specific exception such as IntegrityError or DatabaseError
...
db = sqlite3.connect('db.sqlite3')
db.row_factory = sqlite3.Row cursor = db.cursor()
cursor.execute('''SELECT name, email, phone FROM users''')
for row in cursor:

# row['name'] returns the name column in the query, row['email'] returns email column. print('{0} -> {1}, {2}'.format(
